...
import { AgentInboxInterrupt } from "@/lib/agent-inbox-interrupt";

interface UseInterruptedActionsInput {
  interrupt: AgentInboxInterrupt;
}
...
export default function useInterruptedActions({
  ...
  const toPythonDecision = (response: HumanResponse): Record<string, any> => {
    ...
    if (response.type === "edit") {
      const editArgs =
        typeof response.args === "object" && response.args
          ? (response.args as Record<string, any>)
          : {};
      return {
        type: "edit",
        edited_action: {
          name: editArgs.action,
          args: editArgs.args ?? {},
        },
      };
    }
    ...
    if (response.type === "response") {
      const message =
        typeof response.args === "string" && response.args.trim().length > 0
          ? response.args
  ...
  const resumeRun = (response: HumanResponse[]): boolean => {
    try {
      thread.submit(
        ...
        {
          command: {
            // Python HumanInTheLoopMiddleware expects decisions in approve/edit/reject format.
            resume: { decisions: response.map(toPythonDecision) },
          },
        },
      );
      return true;
